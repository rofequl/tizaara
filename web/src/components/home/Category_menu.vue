              ...
              <ul>
                <li>
                  <div class="menu-title">
                    <h6 class="mb-0">{{ $t("message.category_menu.categories") }}</h6>
                    <router-link to="/categories">{{ $t("message.category_menu.view_all") }}</router-link>
                  </div>
                </li>
                <li v-for="categories in categoryList.slice(0,13)" :key="categories.id">
                  ...
                  <ul v-if="getSubcategoryById(categories.id).length > 0" class="cat-overflow" style="font-size: 15px">
                    <li>
                      <div class="card-columns">
                        <div class="card shadow-none border-0" v-for="subcategories in getSubcategoryById(categories.id).slice(0,6)"
                             :key="subcategories.id">
                          <h6 class="submenu-title cus-mt p-0 pb-2 ml-3" style="border-bottom: 1px solid #efe9e9;font-size: 14px">
                            <router-link :to="'category/'+categories.slug+'/'+subcategories.slug"
                                         :key="subcategories.id">{{ subcategories.name }}
                            </router-link>
                          </h6>
                          <router-link
                              :to="'category/'+categories.slug+'/'+subcategories.slug+'/'+subsubcategories.slug"
                              v-for="subsubcategories in getSubsubcategoryById(subcategories.id).slice(0,5)"
                              :key="subsubcategories.id" class="py-0 my-0 sub-menu-last">{{ subsubcategories.name }}
                          </router-link>
                        </div>
      ...
      <b-container fluid>
        <b-row>
          <b-col style="background-color: #0E63B0" class="rounded py-3 text-white">
            <img src="@/assets/image/rfq_img.png" class="img-fluid bg-light" alt="RFQ Image">
            ...
            <ul class="timeline">
              ...
              <li>
                <p>Receive Verified supplier details
                </p>
              </li>
              <li>
          ...
          <b-col cols="8" class="py-3">
            ...
            <form @submit.prevent="submit" @keydown="form.onKeydown($event)">
              ...
              <div class="form-row mb-2">
                ...
                <div class="form-group col-md-6">
                  <b-form-select v-model="form.unit_id" :options="Object.values(unitList)"
